        ...
        public static MemoryStream GenerateCheckCode(out string checkCode)
        {
            ...
            try
            {
                ...
                for (i = 0; i < checkCode.Length; i++)
                {
                    ...
                    //matrix.RotateAt((float)random1.Next(-15, 10), new PointF(width + sizeF.Width / 2f, single + sizeF.Height / 2f));
                    graphic.Transform = matrix;
                    chr = checkCode[i];
                    graphic.DrawString(chr.ToString(), font, Brushes.Black, new RectangleF(width, single, (float)bitmap.Width, (float)bitmap.Height), stringFormat);
